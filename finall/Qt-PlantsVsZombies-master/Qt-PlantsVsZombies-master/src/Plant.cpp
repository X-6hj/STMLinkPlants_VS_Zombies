...
// ==================== 忧郁菇（GloomShroom） ====================
// 升级型蘑菇，必须种在大喷菇上，3×3范围群体孢子伤害，白天睡觉
GloomShroom::GloomShroom()
{
    ...
    coolTime = 7.5;
    night = true;
    cardGif = "Card/Plants/GloomShroom.png";
    staticGif = "Plants/GloomShroom/GloomShroom.gif";
    normalGif = "Plants/GloomShroom/GloomShroom.gif";
    toolTip = tr("忧郁菇是升级型蘑菇，必须种在大喷菇上\n伤害：中等（群体）\n范围：3×3\n花费：150\n特性：持续对周围3×3范围内的所有僵尸造成孢子伤害\n\n忧郁菇总是看起来很沮丧，但它其实只是在思考哲学问题。它喷射的孢子能覆盖一大片区域，让靠近的僵尸们同时遭殃。");
}

bool GloomShroom::canGrow(int x, int y) const
{
    if (x < 1 || x > 9 || y < 1 || y > scene->getCoordinate().rowCount())
        return false;
    if (scene->isCrater(x, y) || scene->isTombstone(x, y))
        return false;
    // 必须种在大喷菇上，且地面类型兼容（陆地或水域+莲叶）
    int groundType = scene->getGameLevelData()->LF[y];
    QMap<int, PlantInstance *> plants = scene->getPlant(x, y);
    if (!plants.contains(1)) return false;
    if (plants[1]->plantProtoType->eName != "oFumeShroom") return false;
    // 陆地直接允许；水域必须已有莲叶(pKind=0)
    if (groundType == 1) return true;
    if (groundType == 2) return plants.contains(0);
    return false;
}

GloomShroomInstance::GloomShroomInstance(const Plant *plant)
    : PuffShroomInstance(plant)
{
    sleepGif = "Plants/GloomShroom/GloomShroomSleep.gif";
}

void GloomShroomInstance::birth(int c, int r)
{
    // 先铲除该格已存在的大喷菇，再放置自己
    QMap<int, PlantInstance *> plants = plantProtoType->scene->getPlant(c, r);
    if (plants.contains(1)) {
        PlantInstance *old = plants[1];
        if (old->plantProtoType->eName == "oFumeShroom") {
...
void GloomShroomInstance::initTrigger()
{
    // 使用循环定时器，每 2 秒对 3×3 范围进行一次孢子攻击
    QUuid myUuid = uuid;
    (new Timer(picture, 1000, [this, myUuid] {
        ...
        *loop = [this, loop, myUuid] {
            ...
            (new Timer(picture, 2000, *loop))->start();
        };
        (*loop)(); // 立即执行第一次攻击检测
    }))->start();
}

void GloomShroomInstance::doSporeAttack()
{
    Coordinate &coord = plantProtoType->scene->getCoordinate();
    qreal cellCX = coord.getX(col);
    qreal rangeHalf = 120.0;
...
// ==================== 杨桃（Starfruit） ====================
// 五方向攻击：正左、正上、正下、右上、右下，单颗伤害等于普通豌豆
Starfruit::Starfruit()
{
    eName = "oStarfruit";
    cName = tr("杨桃");
    ...
    sunNum = 125;
    coolTime = 7.5;
    cardGif = "Card/Plants/Starfruit.png";
    staticGif = "Plants/Starfruit/Starfruit.gif";
    normalGif = "Plants/Starfruit/Starfruit.gif";
    toolTip = tr("杨桃向5个方向发射星星\n伤害：中等（每颗）\n方向：正左、正上、正下、右上、右下\n花费：125\n\n杨桃的星星形状和它的攻击方式完美契合——向四面八方射出星星子弹，让僵尸无路可逃。");
}

StarfruitInstance::StarfruitInstance(const Plant *plant)
    : PlantInstance(plant)
{
}

void StarfruitInstance::initTrigger()
{
    // 循环定时器，每次发射 5 颗星星，间隔 1.4s
    QUuid myUuid = uuid;
    (new Timer(picture, 500, [this, myUuid] {
        PlantInstance *self = plantProtoType->scene->getPlant(myUuid);
        if (!self || self != this) return;
        ...
        *loop = [this, loop, myUuid] {
            ...
            // 正左 ← (当前行, 向左)
            fireStar(row, attackedLX - 40);

            // 正上 ↑ (上一行, 向左)
            if (row > 1)
                fireStar(row - 1, attackedLX - 30);

            // 正下 ↓ (下一行, 向左)
            if (row < coord.rowCount())
                fireStar(row + 1, attackedLX - 30);

            // 右上 ↗ (上一行, 以 attackedRX 为起点向左)
            if (row > 1)
                fireStar(row - 1, attackedRX - 20);

            // 右下 ↘ (下一行, 以 attackedRX 为起点向左)
            if (row < coord.rowCount())
                fireStar(row + 1, attackedRX - 20);
...
void StarfruitInstance::fireStar(int targetRow, qreal bulletX)
{
    Coordinate &coord = plantProtoType->scene->getCoordinate();
    qreal targetY = coord.getY(targetRow) - 50;
    // 所有星星子弹统一向左发射，伤害=普通豌豆(20)
    (new Bullet(plantProtoType->scene, 4, targetRow,
                attackedLX, bulletX, targetY,
                picture->zValue() + 2, 1))->start();
}
...
PoolCleaner::PoolCleaner()//池塘清扫车
{
    ...
    canEat = 0;
    stature = 1;
    cardGif = "Card/Plants/icon_poolcleaner.png";
...
PotatoMine::PotatoMine()
{
    ...
    staticGif = "Plants/PotatoMine/PotatoMine.gif";
    normalGif = "Plants/PotatoMine/PotatoMine.gif";
    toolTip = tr("土豆地雷需要时间武装自己\n伤害：极高\n范围：1×1\n花费：25\n冷却：15秒\n\n土豆地雷会在种植后14秒内把自己埋入地下，之后随时准备给路过的僵尸一个惊喜。");
}

bool PotatoMine::canGrow(int x, int y) const
{
    if (x < 1 || x > 9 || y < 1 || y > scene->getCoordinate().rowCount())
        return false;
    if (scene->isCrater(x, y) || scene->isTombstone(x, y))
...
PotatoMineInstance::PotatoMineInstance(const Plant *plant)
    : PlantInstance(plant), armed(false), exploding(false)
{
}
...
void PotatoMineInstance::birth(int c, int r)
{
    PlantInstance::birth(c, r);
    // 埋地状态：播放未就绪动画
    picture->setMovie("Plants/PotatoMine/PotatoMineNotReady.gif");
    picture->start();

    // 14秒成长计时器
    QUuid myUuid = uuid;
    (new Timer(picture, 14000, [this, myUuid] {
        PlantInstance *self = plantProtoType->scene->getPlant(myUuid);
        if (!self || self != this) return;
        if (hp <= 0 || exploding) return;
        armed = true;
        picture->setMovie(plantProtoType->normalGif);  // PotatoMine.gif
        picture->start();
    }))->start();
}
...
{
    // 本格触发区：僵尸踏入时引爆
    Trigger *trigger = new Trigger(this, attackedLX - 30, attackedRX + 30, 0, 0);
    triggers.insert(row, QList<Trigger *>{ trigger });
    plantProtoType->scene->addTrigger(row, trigger);
}

void PotatoMineInstance::triggerCheck(ZombieInstance *zombie, Trigger *trigger)
{
    Q_UNUSED(trigger);
    if (!armed || exploding || hp <= 0) return;
    if (!zombie || zombie->hp <= 0 || zombie->goingDie) return;

    // 南瓜头保护：存在则暂不引爆，让僵尸先啃南瓜
    auto plants = plantProtoType->scene->getPlant(col, row);
    if (plants.contains(2)) return;

    // 僵尸踏入本格即触发
    if (zombie->attackedLX >= attackedLX - 30 && zombie->attackedRX <= attackedRX + 30) {
        doExplosion();
    }
}

void PotatoMineInstance::doExplosion()
{
    if (exploding) return;
    exploding = true;
    armed = false;
    ...
    // 阶段1：播放 ExplosionSpudow.gif 爆炸动画
    MoviePixmapItem *explosion = new MoviePixmapItem;
    explosion->setMovie("Plants/PotatoMine/ExplosionSpudow.gif");
    explosion->setPos(cx - 50, cy - 90);
    explosion->setZValue(picture->zValue() + 3);
    plantProtoType->scene->addToGame(explosion);
    explosion->start();

    // 对触碰僵尸造成高额伤害 → 僵尸播放 BoomDie.gif
    QList<ZombieInstance *> zombies = plantProtoType->scene->getZombieOnRowRange(
        row, attackedLX - 40, attackedRX + 40);
    for (auto *z : zombies) {
        if (z->hp > 0 && !z->goingDie)
    ...
    // 阶段2：爆炸动画结束后播放 PotatoMine_mashed.gif，然后销毁
    (new Timer(plantProtoType->scene, 800, [this, explosion, cx, cy] {
        if (explosion->scene()) explosion->scene()->removeItem(explosion);
        delete explosion;

        // 播放碎片动画
        MoviePixmapItem *mashed = new MoviePixmapItem;
        mashed->setMovie("Plants/PotatoMine/PotatoMine_mashed.gif");
        mashed->setPos(cx - 40, cy - 80);
        mashed->setZValue(picture->zValue() + 2);
        plantProtoType->scene->addToGame(mashed);
        mashed->start();

        // 碎片动画播完后清理
        (new Timer(plantProtoType->scene, 600, [this, mashed] {
            if (mashed->scene()) mashed->scene()->removeItem(mashed);
            delete mashed;
            plantProtoType->scene->plantDie(this);
        }))->start();
    }))->start();
...
FlowerPot::FlowerPot()
{
    ...
    staticGif = "Plants/FlowerPot/FlowerPot.gif";
    normalGif = "Plants/FlowerPot/FlowerPot.gif";
    toolTip = tr("花盆让你可以在屋顶上种植植物\n花费：25\n冷却：7.5秒\n只能在屋顶使用\n\n花盆是屋顶关卡的必需品。没有它，大多数植物都无法在坚硬的屋顶上扎根。");
}
...
bool FlowerPot::canGrow(int x, int y) const
{
    ...
    // 仅屋顶空地（LF=3），陆地/水域禁止
    int groundType = scene->getGameLevelData()->LF[y];
    if (groundType != 3) return false;
    QMap<int, PlantInstance *> plants = scene->getPlant(x, y);
    return !plants.contains(0); // 不与其它基座植物共存
}
...
void FlowerPotInstance::getHurt(ZombieInstance *zombie, int aKind, int attack)
{
    PlantInstance::getHurt(zombie, aKind, attack);
    // 花盆被摧毁时，同步销毁上方绑定的植物
    if (hp <= 0) {
        auto plants = plantProtoType->scene->getPlant(col, row);
        if (plants.contains(1)) {
            plantProtoType->scene->plantDie(plants[1]);
        }
    }
}
...
Bullet::Bullet(GameScene *scene, int type, int row, qreal from, qreal x, qreal y, qreal zvalue, int direction, qreal speed)
        ...
        // type:0豌豆 1冰豆 2孢子 3毒气 4星星; speed:默认5,左向10
{
    count = 0;
    QString picName;
    if (type == 4)
        picName = QString("Plants/Star.gif");
    else if (type == 3)
...
Plant *PlantFactory(GameScene *scene, const QString &eName)
{
    ...
    else if (eName == "oGloomShroom")//忧郁菇
        plant = new GloomShroom;
    else if (eName == "oStarfruit")//杨桃
        plant = new Starfruit;
    else if (eName == "oPotatoMine")//土豆地雷
        plant = new PotatoMine;
    else if (eName == "oFlowerPot")//花盆
        plant = new FlowerPot;
    if (plant) {
        plant->scene = scene;
        plant->update();
...
PlantInstance *PlantInstanceFactory(const Plant *plant)
{
    ...
    if (plant->eName == "oPoolCleaner")//池塘清扫车
    ...
    if (plant->eName == "oGloomShroom")//忧郁菇
        return new GloomShroomInstance(plant);
    if (plant->eName == "oStarfruit")//杨桃
        return new StarfruitInstance(plant);
    if (plant->eName == "oPotatoMine")//土豆地雷
        return new PotatoMineInstance(plant);
    if (plant->eName == "oFlowerPot")//花盆
